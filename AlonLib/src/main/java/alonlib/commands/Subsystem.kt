...
/**
 * A robot subsystem: encapsulates a piece of hardware and exposes it to [alonlib.commands.Command]s. The
 * [CommandScheduler] uses each command's [alonlib.commands.Command.requirement] to make sure only one command
 * uses a given subsystem at a time.
 */
interface Subsystem {

	/** Called every scheduler tick, for subsystem-internal state that shouldn't live in a [alonlib.commands.Command]. */
	fun periodic() {}
