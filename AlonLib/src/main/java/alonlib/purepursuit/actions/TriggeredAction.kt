package alonlib.purepursuit.actions

/** An action a [alonlib.purepursuit.Path] fires once [isTriggered] becomes true (e.g. "robot crossed Y=2m"). */
abstract class TriggeredAction {

    private var alreadyPerformed = false

    /** Polled every [alonlib.purepursuit.Path.loop] call; fires [doAction] once [isTriggered] is true. */
    fun loop() {
        if (isTriggered()) {
