...
class Force(force: Number, forceUnit: Unit) : Comparable<Force> {
	...
	init {
		newtons = when (forceUnit) {
			...
			Unit.PoundsForce    -> force.toDouble() * Mass.Companion.KILOGRAMS_PER_POUND * LinearAcceleration.Companion.STANDARD_GRAVITY
			Unit.KilogramsForce -> force.toDouble() * LinearAcceleration.Companion.STANDARD_GRAVITY
	...
	private fun inUnit(forceUnit: Unit) =
		when (forceUnit) {
			...
			Unit.PoundsForce    -> newtons / (Mass.Companion.KILOGRAMS_PER_POUND * LinearAcceleration.Companion.STANDARD_GRAVITY)
			Unit.KilogramsForce -> newtons / LinearAcceleration.Companion.STANDARD_GRAVITY
