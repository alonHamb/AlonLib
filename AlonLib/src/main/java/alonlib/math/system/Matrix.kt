...
class Matrix private constructor(internal val storage: SimpleMatrix) {
    ...
    fun maxAbs(): Double {
        var max = 0.0
        for (r in 0 until rows) for (c in 0 until cols) max = max(max, abs(storage.get(r, c)))
        return max
    }
        ...
        for (i in 0 until n) {
            for (j in 0..i) {
                ...
                if (i == j) {
                    ...
                    l[i, j] = sqrt(sum.coerceAtLeast(0.0))
                } else {
     ...
     * least-squares. Used by e.g. [alonlib.math.kinematics.MecanumDriveKinematics]
    ...
    fun exp(): Matrix {
        ...
        var scaled = this
        if (norm > 0.5) {
            squarings = ceil(ln(norm / 0.5) / ln(2.0)).toInt().coerceAtLeast(0)
