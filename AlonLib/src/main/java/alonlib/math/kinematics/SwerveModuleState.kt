...
class SwerveModuleState(var speedMetersPerSecond: Double = 0.0, var angle: AngularPositon = AngularPositon.kZero) :
	 ...
	 */
	fun optimize(currentAngle: AngularPositon) {
		val delta = angle - currentAngle
		if (abs(delta.degrees) > 90.0) {
			speedMetersPerSecond *= -1.0
			angle = angle.rotateBy(AngularPositon.kPi)
		}
	}
	 ...
	 */
	fun cosineScale(currentAngle: AngularPositon) {
	...
	companion object {
		...
		fun optimize(desiredState: SwerveModuleState, currentAngle: AngularPositon): SwerveModuleState {
			val delta = desiredState.angle - currentAngle
			return if (abs(delta.degrees) > 90.0) {
				SwerveModuleState(-desiredState.speedMetersPerSecond, desiredState.angle.rotateBy(AngularPositon.kPi))
			} else {
