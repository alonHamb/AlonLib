...
class Translation2d(val x: Double = 0.0, val y: Double = 0.0) : Interpolatable<Translation2d> {
	...
	constructor(distance: Double, angle: AngularPositon) : this(distance * angle.cos, distance * angle.sin)

	val norm get() = hypot(x, y)
	val angle get() = AngularPositon(x, y)

	fun getDistance(other: Translation2d) = hypot(other.x - x, other.y - y)

	/** Rotates this translation around the origin by [other]. */
	fun rotateBy(other: AngularPositon) = Translation2d(x * other.cos - y * other.sin, x * other.sin + y * other.cos)

	/** Rotates this translation around [other] by [rotation]. */
	fun rotateAround(other: Translation2d, rotation: AngularPositon) = (this - other).rotateBy(rotation) + other
