...
class ExtentionsTest {
	...
	fun `andThen with a Command unions the requirements of both commands`() {
		...
		assertTrue(firstSub in chained.requirement)
		assertTrue(secondSub in chained.requirement)
	}
	...
	fun `withTimeout wraps the command, preserving its requirements`() {
		...
		assertTrue(sub in command.requirement)
	}
	...
	fun `alongWith and raceWith union the requirements of both commands`() {
		...
		assertTrue(firstSub in along.requirement)
		assertTrue(secondSub in along.requirement)

		val raced = InstantCommand({}, firstSub) raceWith InstantCommand({}, secondSub)
		assertTrue(firstSub in raced.requirement)
		assertTrue(secondSub in raced.requirement)
	}
