 ...
 * command groups for multi-step actions without hand-rolling the state machine.
 */
interface Command {
    ...
    fun hasRequirement(requirement: Subsystem) = requirements.contains(requirement)
    ...
    /** This command run "by proxy" ([ProxyScheduleCommand]), so a command group doesn't inherit its requirements. */
