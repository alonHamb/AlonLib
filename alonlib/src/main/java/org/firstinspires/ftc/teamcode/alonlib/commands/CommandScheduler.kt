...
import org.firstinspires.ftc.teamcode.alonlib.commands.CommandScheduler.registerSubsystem
import org.firstinspires.ftc.teamcode.alonlib.commands.CommandScheduler.run
...
CommandScheduler {
	...
	private val initCommands = mutableListOf<(Command) -> Unit>()
	private val executeCommands = mutableListOf<(Command) -> Unit>()
	private val interruptCommands = mutableListOf<(Command) -> Unit>()
	private val finishCommands = mutableListOf<(Command) -> Unit>()
	...
	private fun initCommand(command: Command, interruptible: Boolean, requirements: Set<Subsystem>) {
		...
		initCommands.forEach { it(command) }
	...
	private fun scheduleOne(interruptible: Boolean, command: Command) {
		...
		val requirements = command.requirement
	...
	fun run() {
		...
		while (iterator.hasNext()) {
			...
			if (!command.runsWhenDisabled() && Robot.isDisabled) {
				command.end(true)
				interruptCommands.forEach { it(command) }
				requirementOwners.keys.removeAll(command.requirement)
				iterator.remove()
				continue
			}

			command.execute()
			executeCommands.forEach { it(command) }
			if (command.isFinished()) {
				command.end(false)
				finishCommands.forEach { it(command) }
				iterator.remove()
				requirementOwners.keys.removeAll(command.requirement)
			}
		}
	...
	fun setDefaultCommand(subsystem: Subsystem, defaultCommand: Command) {
		require(subsystem in defaultCommand.requirement) { "Default commands must require their subsystem!" }
	...
	fun onCommandInitialize(action: (Command) -> Unit) = initCommands.add(action)
	fun onCommandExecute(action: (Command) -> Unit) = executeCommands.add(action)
	fun onCommandInterrupt(action: (Command) -> Unit) = interruptCommands.add(action)
	fun onCommandFinish(action: (Command) -> Unit) = finishCommands.add(action)
}
