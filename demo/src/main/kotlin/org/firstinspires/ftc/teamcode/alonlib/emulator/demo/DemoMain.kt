package org.firstinspires.ftc.teamcode.alonlib.emulator.demo
...
/**
 * A minimal "robot" wired up purely to give this repo a standalone window to run and click
 * around -- the same [emulator.ui.RunnerShellApp] shell, field view, port monitor, telemetry, and
 * gamepad handling that https://github.com/alonHamb/ftc-control-hub-emulator's own `:demo` module
 * shows. `alonlib-emulator`'s `EmulatedRobot.launch()` drives this exact same UI against a real
 * unmodified OpMode's hardware map -- see the root README's "Running on a desktop emulator instead
 * of a robot" section for that.
 */
fun main() {
