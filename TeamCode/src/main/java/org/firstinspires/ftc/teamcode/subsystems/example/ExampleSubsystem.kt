...
class ExampleSubsystem(hardwareMap: HardwareMap, var telemetry: Telemetry, var telemetryLevel: TelemetryLevel) : SubsystemBase() {
    ...
    val encoder = HaMotor(hardwareMap, "motor 0", 8192, 312).apply { runningDirection = Motor.Direction.REVERSE }
    val servo = HaServo(hardwareMap, EXAMPLE_SERVO_ID, EXAMPLE_SERVO_RANGE).apply { runningDirection = Motor.Direction.REVERSE }
    ...
    fun setExamplePosition(position: Double) {
        servo.position = position.rotations
